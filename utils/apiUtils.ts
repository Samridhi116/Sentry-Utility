...
export async function fetchApiData<T>(page: Page, url: string): Promise<ApiResponse<T>> {
  try {
    ...
    let nextCursor: string | null = data.meta?.cursor || data.links?.next?.cursor || data.links?.next?.results || null;
    if (!nextCursor && data.links?.next && typeof data.links.next === 'string') {
      try {
        const nextUrl = new URL(data.links.next, 'https://us.sentry.io');
    ...
    if (!nextCursor && data.links?.next && data.links.next.includes('cursor=')) {
